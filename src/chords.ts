// https://i.pinimg.com/originals/26/87/45/26874503574c2e49acb2f05fca523101.jpg
import { Note, NoteSettings } from "./notes";
...
export const hasAllNotes = (list: Set<string>, notes: Note[]) => {
...
export const getChord = (...notes: NoteSettings[]): null | Note => {
    if (notes.length >= 3) {
        const noteValues = notes.map((note: NoteSettings) => {
            return note.name;
        });
        const allChords = Object.keys(Chords) as Array<Note>;
        let targetChord: null | Note = null;
        allChords.some((name: Note) => {
            const found = hasAllNotes(Chords[name], noteValues);
